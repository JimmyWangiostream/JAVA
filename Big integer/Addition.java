...
public String perform(String num1, String num2) { 
	...
	for(int i=0;i<len;i++){
	 ...
	 //System.out.println(abcom);
	  if((n1l-n1s)>(n2l-n2s)){abcom=1;}
	   ...
	   if(abcom==0&&(n1s==1&&n2s==0)){state=0;}//-9 + 9
	   if(abcom==0&&(n1s==0&&n2s==1)){state=0;}//9 + -9 
	...
	if(num1.equals(num2)&&n1zero+n2zero==0){state=5;}
	...
	if(state==1||state==2||state==3||state==4){
		 if(state==1||state==3){//1-2of minus			
		int i = 0,count=0;
		  int borrow = 0;
	        for(i = 0; i < len+1; ++i){
		 ...
		 if(state==2||state==4){//2-1 of minus
			 int i = 0,count=0;
			  int borrow = 0;
		        for(i = 0; i < len+1; ++i){
	...
	if(state==5||state==6||state==7||state==8){//add
